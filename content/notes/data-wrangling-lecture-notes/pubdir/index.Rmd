...
fwrite(x = data_raw, file = "data/wrangling_slides.csv")

swiid_red<- fread(file = "data/swiid9_2_summary.csv") %>%
  filter(
    country %in% c("Germany", "Greece"),
    year %in% c(2017, 2018)
  ) %>%
...
fwrite(x = swiid_red, file = "data/wrangling_slides_gini.csv")

swiid_join <- fread(file = "data/swiid9_2_summary.csv") %>%
  filter(
    country %in% c("Greece"),
    year %in% c(2015, 2017)
  ) %>%
...
fwrite(x = swiid_join, file = "data/wrangling_slides_gini_grc.csv")
```
...
fwrite(x = data_final_expl, 
       file = "data/wrangling_slides_final_expl.csv")
```

```{r, include=FALSE}
gini_red <- fread(file = "data/wrangling_slides_gini.csv")
data_raw <- fread(file = "data/wrangling_slides.csv")
data_raw_long <- fread(file = "data/wrangling_slide_long.csv")
data_final_expl <- fread(file = "data/wrangling_slides_final_expl.csv")
swiid_join <- fread(file = "data/wrangling_slides_gini_grc.csv")
```
...
```{r, echo=FALSE}
pipe_data_raw %>%
  pivot_longer(cols = all_of(c("gdp", "unemp"))) %>%
...
```{r, include=FALSE}
data_unemp <- data_raw %>%
  select(all_of(c("country", "year", "unemp"))) %>%
