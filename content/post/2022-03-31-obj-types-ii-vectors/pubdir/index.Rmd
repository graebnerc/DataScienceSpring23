...
See the [previous post](/post/object-types-func/) 
for a treatment of functions, and the 
[upcoming one](/post/object-types-adv/) 
...
`TRUE` und `FALSE`^[While you can abbrevate the two with `T` and `F`, respectively,
I recommend against using these sometimes ambiguous abbrevations.]
...
show unintuitive behavior when this category is used. I recommend you to never
use it.
* `character` (words): these can contain all kinds of tokens and are characterized
by the fact that they always start and end with `"`. Examples would be
`"Hello"`, `"500"` oder `"1_2_Three"`.
...
store observations of subjects, and for some subjects the observation is 
...
You will transform objects yourself especially if you want to use a certain 
...
In many cases, function do the necessary transformations of their arguments
...
There is no such concept in R: `2` is a vector with one elements 
(or: of length 1).
...
Since atomic vectos can only contain objects of the same type, one might expect
...
hierarchy discussed above:  `logical` &rarr; `integer` &rarr; `double` &rarr; `character`. 
Due to the absence of errors or warning messagse, such operations are often
an important source for mistakes.
...
To do so we must use the package^[We will learn about packages in later session.]
`bit64`:
...
### Logical operations {#basics-logic}
...
same length remains concealed. An example of this is the following code, in which
...
useful in statistical context, where are particular element of a vector cannot
...
R choose the necessary space between the elements, we better use `length.out`:
...
If we are looking for the **largest** and **smalles** value of a vector we can
use `min()` and `max()`:
...
The **mean** or the **variance/standard deviation** of the elemtns can be 
computed with `mean()`, `var()`, bzw. `sd()`, all of which have also the
...
1. Vectorization does not work for list
...
Then to extract an elemtn of this vector we can chain the brackets:
...
something as an atomic vector, you should do so. For using lists, you should 
