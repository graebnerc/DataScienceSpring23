...
communicating with your computer in the language `R`: if you you do not follow 
the right syntax, i.e. you 'speak' gramatically incorrect, your computer will 
...
R community is one of the main reasons for success and wide applicability of R.
...
across different projects, document them, and make them available too use 
...
usually called `base R`, because we can use all the functionalities easily
immediately after installing R on our computer.

The function `assign`, for example, is part of `base R`: we start R and 
...
that contains the function definition on our computer.
...
on this platform can also be installed directly from repository they were
...
For example, if we want to install the package `ineq` this is done with the
following command: 
...
If we ommited the `::`, R would not look into the package `ineq` and, therefore,
was not able to find the function, returning an error:
...
This way you can see immediately which packages have to be installed for the
script to work.
...
Since in R a name can only belong to one object, names may be overwritten or 
...
In this case, always use `plm::lead` and `dplyr::lead`, even it it is not 
